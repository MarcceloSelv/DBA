...
SELECT
	...
	'CREATE NONCLUSTERED INDEX ix_IndexName ON ' + sys.objects.name COLLATE DATABASE_DEFAULT + ' ( ' + IsNull(sys.dm_db_missing_index_details.equality_columns, '') + CASE WHEN sys.dm_db_missing_index_details.inequality_columns IS NULL 
                THEN ''  
	    ELSE CASE WHEN sys.dm_db_missing_index_details.equality_columns IS NULL 
			     THEN ''  
		ELSE ',' END + sys.dm_db_missing_index_details.inequality_columns END + ' ) ' + CASE WHEN sys.dm_db_missing_index_details.included_columns IS NULL 
			THEN ''  
	    ELSE 'INCLUDE (' + sys.dm_db_missing_index_details.included_columns + ')' END + ';' AS CreateIndexStatement,
