...
(
	...
	and execution_count > 15
    group by query_hash
    order by sum(total_logical_reads + total_logical_writes) / (sum(execution_count) + 0.0) desc
)
select @@servername as servername,
    coalesce(db_name(st.dbid), db_name(ST.dbid), 'Resource') AS [DatabaseName],
...
order by fq.io / (executionCount + 0.0) desc,
    fq.query_hash,
