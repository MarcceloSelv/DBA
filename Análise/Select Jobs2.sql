SELECT 
	job_name		= J.value('(./@job_name)[1]', 'VARCHAR(150)')
	, is_job_enabled	= J.value('(./@is_job_enabled)[1]', 'VARCHAR(3)')
	, is_schedule_enabled	= J.value('(./@is_schedule_enabled)[1]', 'VARCHAR(3)')
	, date_modified		= J.value('(./@date_modified)[1]', 'datetime')
	, version_number	= J.value('(./@version_number)[1]', 'int')
	, schedule_name		= J.value('(ss/@schedule_name)[1]', 'VARCHAR(100)')
	, [Description]		= J.value('(ss/@Description)[1]', 'VARCHAR(1000)')
	, database_name		= step.value('(./@database_name)[1]', 'VARCHAR(50)')
	, step_name		= step.value('(./@step_name)[1]', 'VARCHAR(50)')
	--, command = step.value('(./@command)[1]', 'VARCHAR(MAX)')
	, last_run_date		= step.value('(./@last_run_date)[1]', 'VARCHAR(15)')
	, subsystem		= step.value('(./@subsystem)[1]', 'VARCHAR(10)')
	, command		= CAST((SELECT step.value('(./@command)[1]', 'VARCHAR(MAX)') as [processing-instruction(x)] FOR XML PATH('')) AS XML)
FROM (
SELECT Xml_column = CAST(Xml_column as xml)
FROM (SELECT Xml_column = (
SELECT
--SJ.job_id 
job_name = sj.name
...
CROSS APPLY X.Xml_column.nodes('/jobs/sj') as Jobs(J)
CROSS APPLY J.nodes('ss/step') as Schedules(step)