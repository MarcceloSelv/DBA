...
-- 10.25.9501		RTM Service Update 6
...
FROM ECARGO.sys.database_files OPTION (RECOMPILE);
...
SELECT DB_NAME(DB_ID('ECARGO')) AS [Database Name],[file_id], num_of_reads, num_of_writes, 
...
FROM sys.dm_io_virtual_file_stats(DB_ID('ECARGO'), NULL) OPTION (RECOMPILE);
...
ON p.[object_id] = qs.[object_id]
WHERE qs.database_id = DB_ID('ECARGO')
ORDER BY qs.total_logical_reads --/qs.execution_count
...
SELECT TOP(50) OBJECT_NAME(qt.objectid, qt.dbid) AS [SP Name],
...
INNER JOIN ECARGO.sys.indexes AS i WITH (NOLOCK)
ON s.[object_id] = i.[object_id]
AND i.index_id = s.index_id
WHERE OBJECTPROPERTY(s.[object_id],'IsUserTable') = 1
AND s.database_id = DB_ID('ECARGO')
...
WHERE mid.database_id = DB_ID('ECARGO') -- Remove this to see for entire instance
...
SELECT OBJECT_NAME(p.[object_id]) AS [ObjectName], COUNT(*)/128 AS [Buffer size(MB)],  COUNT(*) AS [BufferCount] 
