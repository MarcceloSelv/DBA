...
IF (CHARINDEX('7.00.',@@VERSION)<>0) OR (CHARINDEX('8.00.',@@VERSION)<>0) BEGIN
	IF (OBJECT_ID('dbo.sp_sqldiag07') IS NULL) RAISERROR('Error creating sp_sqldiag07',16,127)
	...
	IF (CHARINDEX('8.00.',@@VERSION)<>0)
		...
		IF (OBJECT_ID('dbo.sp_sqldiag08') IS NULL) RAISERROR('Error renaming sp_sqldiag07 to sp_sqldiag08',16,127)
	END
END
GO
...
IF (CHARINDEX('9.00.',@@VERSION)<>0) AND (OBJECT_ID('sp_sqldiag09') IS NULL) 
...
IF (CHARINDEX('7.00.',@@VERSION)<>0) AND (OBJECT_ID('dbo.sp_trace07') IS NULL) 
...
IF @Events IS NOT NULL BEGIN -- Loop through the @Events and @Cols strings, parsing out each event & column number and adding them to the trace definition
  ...
  WHILE @i<>0 BEGIN
    ...
    WHILE @j<>0 BEGIN
      SET @Col=CAST(LEFT(@ColStr,@j-1) AS int)
	  if (@Col<>1) OR (not (@Event IN (10,11,146)))
		EXEC sp_trace_setevent @TraceId=@QueueHandle, @eventid=@Event, @columnid=@Col, @on=@On
	  else
...
RETURN 0
GO
IF (CHARINDEX('9.00.',@@VERSION)<>0) AND (OBJECT_ID('dbo.sp_trace09') IS NULL) 
...
GO
IF (CHARINDEX('9.00.',@@VERSION)<>0) AND (OBJECT_ID('dbo.sp_sqldiag_cleanup09') IS NULL) 
