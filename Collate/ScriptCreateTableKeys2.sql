...
/*
If Object_ID('ScriptCreateTableKeys', 'P') Is Not Null
        Drop Procedure ScriptCreateTableKeys absorcao_xml
*/
go
...
*/
alter PROC [dbo].[ScriptCreateTableKeys]
    @table_name SYSNAME
AS
BEGIN
    SET NOCOUNT ON
    ...
    SET @sql = '' +
        'SELECT ' +
            ...
            ''') '' + @crlf + ' +
            '''ON '' + ds.data_space + '';'' + ' +
                '@crlf + @crlf COLLATE database_default + ''go'' + @crlf AS [-- Create Candidate Keys] ' +
...
(
        ...
        ');' + 
            @crlf + @crlf COLLATE database_default + 'go' + @crlf AS [-- Create Referencing FKs]
...
Grant Execute On dbo.ScriptCreateTableKeys To Public
go
Grant Execute On dbo.ScriptCreateTableKeys To Sistema
go
