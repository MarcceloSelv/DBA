...
/*
If Object_ID('ScriptDropTableKeys', 'P') Is Not Null
        Drop Procedure ScriptDropTableKeys DOCTO_TRANSPORTE
*/
go
 
alter PROC [dbo].[ScriptDropTableKeys]
    @table_name SYSNAME
AS
BEGIN
    SET NOCOUNT ON
    ...
    SELECT
        ...
        'DROP CONSTRAINT ' + QUOTENAME(name) + ';' + @crlf +'go' +
            @crlf COLLATE database_default AS [-- Drop Referencing FKs]
                ...
                'CASE ' +
                    'WHEN 1 IN (i.is_unique_constraint, i.is_primary_key) THEN ' +
                        ...
                        '''DROP CONSTRAINT '' + QUOTENAME(i.name) + '';'' + ' +
                            '@crlf COLLATE database_default + ''go'' + @crlf ' +
                    'ELSE ' +
...
Grant Execute On dbo.ScriptDropTableKeys To Public
go
Grant Execute On dbo.ScriptDropTableKeys To Sistema
go
